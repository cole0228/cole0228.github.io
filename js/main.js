 ...
 (function() {
	...
	function init() {
		...
		if( sectionsCount >= 3 && Modernizr.csstransforms3d ) {
			...
			classie.add( container, 'triplelayout' );
			...
			function navigate( dir ) {
				...
				classie.add( container, 'move-' + invDir );
				...
				nextLink.className = '';
				classie.add( nextSection, '-' + dir + 'outer' );
				classie.add( nextLink, 'nav-' + dir + 'outer' );
				...
				var onTransitionEndFn = function() {
					...
					if( dir === 'right' ) {
						classie.add( sections[ current ], 'left' );
						classie.add( sections[ r ], 'current' );
						classie.add( nextSection, 'right' );

						classie.add( links[ current ], 'nav-left' );
						classie.add( links[ r ], 'nav-current' );
						classie.add( nextLink, 'nav-right' );

						current = current < sectionsCount - 1 ? current + 1 : 0;
					}
					else if( dir === 'left' ) {
						classie.add( nextSection, '-left' );
						classie.add( sections[ l ], 'current' );
						classie.add( sections[ current ], 'right' );

						classie.add( nextLink, 'nav-left' );
						classie.add( links[ l ], 'nav-current' );
						classie.add( links[ current ], 'nav-right' );

						current = current > 0 ? current - 1 : sectionsCount - 1;
					}

					classie.remove( container, 'move-' + invDir );
					isAnimating = false;
				}
			...
			classie.add( sections[ current ], 'current' );
			classie.add( sections[ current + 1 ], 'right' );
			classie.add( sections[ sectionsCount - 1 ], '-left' );
			// same for the header links
			classie.add( links[ current ], 'nav-current' );
			classie.add( links[ current + 1 ], 'nav-right' );
			classie.add( links[ sectionsCount - 1 ], 'nav-left' );
			...
			links.forEach( function( el, i ) {
				el.addEventListener( eventtype, function( ev ) {
					ev.preventDefault();
					if( classie.has( el, 'nav-right' ) ) {
						navigate( 'right' );
					}
					else if ( classie.has( el, 'nav-left' ) ) {
						navigate( 'left' );
					}
					else {
						return false;
